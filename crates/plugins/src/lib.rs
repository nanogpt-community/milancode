...
const MANIFEST_DIR_NAME: &str = ".milancode-plugin";
...
impl PluginTool {
    ...
    pub fn execute(&self, input: &Value) -> Result<String, PluginError> {
        ...
        process
            ...
            .stdout(Stdio::piped())
            .stderr(Stdio::piped())
            .env("MILANCODE_PLUGIN_ID", &self.plugin_id)
            .env("MILANCODE_PLUGIN_NAME", &self.plugin_name)
            .env("MILANCODE_TOOL_NAME", &self.definition.name)
            .env("MILANCODE_TOOL_INPUT", &input_json);
        if let Some(root) = &self.root {
            process
                .current_dir(root)
                .env("MILANCODE_PLUGIN_ROOT", root.display().to_string());
        }
...
fn run_lifecycle_commands(
    ...
    for command in commands {
        ...
        process.env("MILANCODE_PLUGIN_ID", &metadata.id);
        process.env("MILANCODE_PLUGIN_NAME", &metadata.name);
        process.env("MILANCODE_PLUGIN_STAGE", stage);
...
mod tests {
    ...
    fn temp_dir(label: &str) -> PathBuf {
        let nanos = SystemTime::now()
            ...
            .as_nanos();
        std::env::temp_dir().join(format!("milancode-plugins-{label}-{nanos}"))
    }
