...
const SLASH_COMMAND_SPECS: &[SlashCommandSpec] = &[
    ...
    SlashCommandSpec {
        ...
        category: SlashCommandCategory::Core,
        detail: "With no argument, opens the auth service picker. Removes saved credentials from `~/.milancode/credentials.json`, but shell environment variables still take precedence if set.",
        resume_supported: false,
    },
    SlashCommandSpec {
        name: "mcp",
        ...
        detail: "Use `/mcp add <name>` to scaffold a server entry in `.milancode/settings.json`, then `/mcp enable <name>` or `/mcp disable <name>` to manage local overrides in `.milancode/settings.local.json`.",
        resume_supported: false,
    },
    SlashCommandSpec {
        name: "permissions",
    ...
    SlashCommandSpec {
        name: "config",
        aliases: &[],
        summary: "Inspect MilanCode config files or merged sections",
    ...
    SlashCommandSpec {
        name: "memory",
        aliases: &[],
        summary: "Inspect loaded MilanCode instruction memory files",
        argument_hint: None,
        category: SlashCommandCategory::Workspace,
    ...
    SlashCommandSpec {
        name: "init",
        aliases: &[],
        summary: "Create a starter MILANCODE.md for this repo",
        argument_hint: None,
        category: SlashCommandCategory::Workspace,
        detail: "Bootstraps common project-local MilanCode files for a repository.",
        resume_supported: true,
    },
    SlashCommandSpec {
        name: "diff",
    ...
    SlashCommandSpec {
        ...
        argument_hint: None,
        category: SlashCommandCategory::Core,
        detail: "Prints the MilanCode build version and related metadata.",
        resume_supported: true,
    },
    SlashCommandSpec {
        name: "branch",
    ...
    SlashCommandSpec {
        ...
        summary: "Manage MilanCode plugins",
        argument_hint: Some(
    ...
    SlashCommandSpec {
        name: "agents",
        aliases: &[],
        summary: "List configured MilanCode agents",
    ...
    SlashCommandSpec {
        name: "skills",
        aliases: &[],
        summary: "List available MilanCode skills",
        argument_hint: Some("[list|help|init <name>]"),
        category: SlashCommandCategory::Workspace,
        detail: "Use `/skills init <name>` to scaffold `.milancode/skills/<name>/SKILL.md` in the current project.",
        resume_supported: true,
    },
];
...
const HELP_TOPICS: &[HelpTopic] = &[
    ...
    HelpTopic {
        ...
        detail: "Run `/login` or `/auth` with no argument to open a service picker, or `/logout [service]` to remove saved credentials. Supported services are `nanogpt`, `synthetic`, `openai-codex`, `opencode-go`, and `exa`. API keys are stored in `~/.milancode/credentials.json`; `openai-codex` stores device-code auth tokens there instead, and shell-scoped env vars still take precedence.",
    },
    HelpTopic {
        name: "sessions",
    ...
    HelpTopic {
        ...
        detail: "Use `/skills init <name>` to scaffold a project skill, `/mcp add <name>` to add a starter MCP server entry to `.milancode/settings.json`, `/mcp enable <name>` or `/mcp disable <name>` for local MCP overrides, and `/plugins help` for local plugin install and enable guidance.",
    },
    HelpTopic {
        name: "web",
        summary: "Provider-agnostic web search and scrape",
        detail: "MilanCode always uses Exa for `WebSearch` and `WebScrape`. Configure `EXA_API_KEY` or store Exa credentials through `/login exa`.",
    },
];
...
impl DefinitionSource {
    fn label(self) -> &'static str {
        match self {
            Self::Project => "Project (.milancode)",
            Self::UserConfigHome => "User (MILANCODE_CONFIG_HOME)",
        }
    }
}
...
fn scaffold_skill(args: &str, cwd: &Path) -> io::Result<String> {
    ...
    if name.contains('/') || name.contains('\\') {
        return Ok(format!(
            "Skills\n  error: invalid skill name `{name}`\n  hint: skill names become directory names under .milancode/skills/"
        ));
    }

    let skill_dir = cwd.join(".milancode").join("skills").join(name);
...
fn discover_definition_roots(cwd: &Path, leaf: &str) -> Vec<(DefinitionSource, PathBuf)> {
    ...
    for ancestor in cwd.ancestors() {
        push_unique_root(
            ...
            ancestor.join(".milancode").join(leaf),
        );
    }

    if let Some(config_home) = milancode_config_home() {
        push_unique_root(
            &mut roots,
...
fn discover_skill_roots(cwd: &Path) -> Vec<SkillRoot> {
    ...
    for ancestor in cwd.ancestors() {
        push_unique_skill_root(
            &mut roots,
            DefinitionSource::Project,
            ancestor.join(".milancode").join("skills"),
            SkillOrigin::SkillsDir,
        );
        push_unique_skill_root(
            &mut roots,
            DefinitionSource::Project,
            ancestor.join(".milancode").join("commands"),
...
fn render_skills_usage(args: Option<&str>) -> String {
    let mut lines = vec![
        ...
        "  Init             Create .milancode/skills/<name>/SKILL.md in the current project"
            .to_string(),
    ];
    if let Some(args) = args {
        lines.push(format!("  Unsupported      {}", args.trim()));
