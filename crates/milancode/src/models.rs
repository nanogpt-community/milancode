...
use api::{
    resolve_api_key_for, resolve_base_url_for, ApiService, ModelCapabilities, ModelInfo,
    ModelProvider, NanoGptClient, ProviderPrice,
};
...
const AVAILABLE_SERVICES: [ApiService; 1] = [ApiService::NanoGpt];
...
pub fn infer_service_for_model(_model: &str) -> ApiService {
    ApiService::NanoGpt
}
...
pub fn open_model_picker() -> Result<ModelSelection, Box<dyn std::error::Error>> {
    ...
    if models.is_empty() {
        return Err("no models were returned by MilanCode".into());
    }
...
fn fetch_service_models(
    ...
    fetch_service_models_via_api(service)
}
...
fn state_path() -> Result<PathBuf, Box<dyn std::error::Error>> {
    Ok(milancode_config_home()?.join("state.json"))
}
...
mod tests {
    ...
    #[test]
    fn infers_nanogpt_service_for_model_prefixes() {
        assert_eq!(
