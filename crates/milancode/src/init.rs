...
const GITIGNORE_COMMENT: &str = "# MilanCode local artifacts";
...
pub(crate) fn initialize_repo(cwd: &Path) -> Result<InitReport, Box<dyn std::error::Error>> {
    initialize_repo_with_milancode_md(cwd, &render_init_milancode_md(cwd))
}

pub(crate) fn initialize_repo_with_milancode_md(
    cwd: &Path,
    milancode_md_content: &str,
) -> Result<InitReport, Box<dyn std::error::Error>> {
    let mut artifacts = Vec::new();

    let milancode_dir = cwd.join(".milancode");
    artifacts.push(InitArtifact {
        name: ".milancode/",
        status: ensure_dir(&milancode_dir)?,
    });

    let settings_local = milancode_dir.join("settings.local.json");
    artifacts.push(InitArtifact {
        name: ".milancode/settings.local.json",
    ...
    let milancode_md = cwd.join("MILANCODE.md");
    artifacts.push(InitArtifact {
        name: "MILANCODE.md",
        status: write_file_if_missing(&milancode_md, milancode_md_content)?,
    });
...
pub(crate) fn render_init_milancode_md(cwd: &Path) -> String {
    let detection = detect_repo(cwd);
    let mut lines = vec![
        "# MILANCODE.md".to_string(),
        String::new(),
        "This file gives MilanCode repo-specific guidance for working in this project.".to_string(),
        String::new(),
    ];
    ...
    lines.push("- Keep shared defaults in `.milancode/settings.json`; reserve `.milancode/settings.local.json` for machine-local overrides.".to_string());
    lines.push(
        "- Update `MILANCODE.md` intentionally when repo workflows or conventions change.".to_string(),
    );
    lines.push(String::new());
...
#[cfg(test)]
mod tests {
    use super::{initialize_repo, initialize_repo_with_milancode_md, render_init_milancode_md};

    #[test]
    fn renders_milancode_md_header() {
        let rendered = render_init_milancode_md(std::path::Path::new("."));
        assert!(rendered.contains("# MILANCODE.md"));
        assert!(rendered.contains("MilanCode"));
    }

    #[test]
    fn initialize_repo_creates_milancode_artifacts() {
        ...
        assert!(report
            .artifacts
            .iter()
            .any(|artifact| artifact.name == "MILANCODE.md"));
        assert!(root.join("MILANCODE.md").is_file());
        assert!(root.join(".milancode").is_dir());

        std::fs::remove_dir_all(&root).expect("temp root should be removable");
    }

    #[test]
    fn initialize_repo_writes_custom_milancode_md() {
        let root = std::env::temp_dir().join(format!(
            "milancode-init-custom-{}",
            std::time::SystemTime::now()
                .duration_since(std::time::UNIX_EPOCH)
                .expect("time should work")
                .as_nanos()
        ...
        initialize_repo_with_milancode_md(&root, "# MILANCODE.md\n\nCustom project guidance\n")
            .expect("init should succeed");
        let written = std::fs::read_to_string(root.join("MILANCODE.md")).expect("milancode md exists");
