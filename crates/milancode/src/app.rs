...
use api::{
    ...
    ReasoningEffort, StreamEvent as ApiStreamEvent, ThinkingConfig, ToolChoice, ToolDefinition,
...
use platform::milancode_config_home as resolve_milancode_config_home;
...
use crate::init::{initialize_repo, initialize_repo_with_milancode_md, render_init_milancode_md};
use crate::input;
use crate::models::{
...
const INIT_MILANCODE_MD_MAX_TOKENS: u32 = 2_048;
...
const BUILD_TARGET: Option<&str> = option_env!("MILANCODE_BUILD_TARGET");
const IMAGE_REF_PREFIX: &str = "@";
const SELF_UPDATE_REPOSITORY: &str = "nanogpt-community/milancode";
const SELF_UPDATE_LATEST_RELEASE_URL: &str =
    "https://api.github.com/repos/nanogpt-community/milancode/releases/latest";
const SELF_UPDATE_USER_AGENT: &str = "milancode-self-update";
...
fn parse_login_service(value: &str) -> Result<AuthService, String> {
    match value.trim().to_ascii_lowercase().as_str() {
        ...
        other => Err(format!(
            "unsupported login service `{other}`; expected nanogpt or exa"
        )),
    }
}
...
fn print_bootstrap_plan() {
    for phase in runtime::BootstrapPlan::milancode_default().phases() {
        println!("- {phase:?}");
    }
}
...
fn add_mcp_server_interactive(
    ...
    let settings_dir = cwd.join(".milancode");
...
fn save_credentials(
    ...
    let config_home = milancode_config_home()?;
...
fn remove_saved_credentials(
    ...
    let credentials_path = milancode_config_home()?.join("credentials.json");
...
fn milancode_config_home() -> Result<PathBuf, Box<dyn std::error::Error>> {
    resolve_milancode_config_home()
        .ok_or_else(|| "could not resolve MILANCODE_CONFIG_HOME, HOME, or USERPROFILE".into())
}
...
fn permission_mode_from_env() -> Option<PermissionMode> {
    env::var("MILANCODE_PERMISSION_MODE")
        .ok()
        .as_deref()
...
fn load_mcp_catalog(cwd: &Path) -> Result<McpCatalog, Box<dyn std::error::Error>> {
    ...
    for (server_name, scoped) in servers {
        ...
        match scoped.transport() {
            ...
            other => {
                status.note = format!(
                    "{:?} transport is configured but not executable in MilanCode yet",
                    other
                );
            }
        }
    ...
    runtime.block_on(async move {
        let initialize = http_jsonrpc_request::<JsonValue>(
            ...
            Some(serde_json::json!({
                ...
                "capabilities": {"roots": {}},
                "clientInfo": {"name": "milancode", "version": VERSION}
            })),
        )
        .await?;
        if let Some(error) = initialize.error {
...
fn call_mcp_tool(
    ...
    match binding.config.transport() {
        ...
        other => Err(format!("MCP transport {:?} is not executable in MilanCode yet", other).into()),
    }
}
...
fn print_mcp_status(catalog: &McpCatalog) {
    if catalog.servers.is_empty() {
        ...
        println!(
            "  {} add `mcpServers` to `.milancode/settings.json` to expose MCP tools",
            report_label("hint:")
        );
        return;
    }
...
fn resolve_exa_api_key() -> Result<String, Box<dyn std::error::Error>> {
    match env::var("EXA_API_KEY") {
        ...
        Err(env::VarError::NotPresent) => {
            let path = milancode_config_home()?.join("credentials.json");
...
fn sessions_dir() -> Result<PathBuf, Box<dyn std::error::Error>> {
    let cwd = env::current_dir()?;
    let path = cwd.join(".milancode").join("sessions");
...
fn render_memory_report() -> Result<String, Box<dyn std::error::Error>> {
    ...
    if project_context.memory_files.is_empty() {
        lines.push("  No `.milancode/memory` files discovered.".to_string());
    } else {
...
fn render_repl_help() -> String {
    let intro = ui::panel(
        ...
        &[
            ...
            ui::PanelRow::Line(
                "• supported auth services: nanogpt, exa"
                    .to_string(),
            ),
            ui::PanelRow::Line(
...
fn custom_command_roots(cwd: &Path) -> Vec<PathBuf> {
    ...
    if let Some(config_home) = resolve_milancode_config_home() {
        roots.push(config_home.join("commands"));
    }
    for ancestor in cwd.ancestors().collect::<Vec<_>>().into_iter().rev() {
        roots.push(ancestor.join(".milancode").join("commands"));
    }
    roots
}
...
struct LiveCli {
    ...
    proxy_tool_calls: bool,
    mcp_catalog: McpCatalog,
    runtime: ConversationRuntime<MilanCodeRuntimeClient, CliToolExecutor>,
...
fn build_runtime(
    ...
    fast_mode: FastMode,
    render_model_output: bool,
) -> Result<ConversationRuntime<MilanCodeRuntimeClient, CliToolExecutor>, Box<dyn std::error::Error>> {
...
fn configured_auto_compaction_threshold() -> Option<u32> {
    std::env::var("MILANCODE_AUTO_COMPACT_INPUT_TOKENS")
        .ok()
...
impl ApiClient for MilanCodeRuntimeClient {
    fn stream(&mut self, request: ApiRequest) -> Result<Vec<AssistantEvent>, RuntimeError> {
        ...
        let message_request = MessageRequest {
            ...
            thinking: effective_reasoning_effort
                .is_some()
        ...
        self.runtime.block_on(async {
            ...
            // Print the "● milancode" assistant lead exactly once per streamed
            ...
            loop {
                let event = match stream.next_event().await {
                    ...
                    Err(ApiError::StreamApi {
                        ...
                        eprintln!(
                            "[milancode] streaming failed with invalid_response_error{}; retrying non-streaming",
                            message
                                .as_deref()
...
fn session_project_root(session_path: Option<&Path>) -> Option<PathBuf> {
    ...
    let milancode_dir = sessions_dir.parent()?;
    (milancode_dir.file_name()? == ".milancode").then(|| milancode_dir.parent().map(Path::to_path_buf))?
}
...
fn print_help() {
    ...
    println!("Usage");
    println!(
        "  milancode [--model MODEL] [--permission-mode MODE] [--mode MODE] [--reasoning LEVEL] [--fast] [--allowedTools TOOL[,TOOL...]]"
    );
    println!("                                               Start interactive REPL");
    println!(
        "  milancode login [SERVICE] [--api-key KEY]    Save credentials for NanoGPT or Exa"
    );
    println!(
        "                                               Services: nanogpt, exa"
    );
    println!("  milancode logout [SERVICE]                   Remove saved credentials for a service");
    println!("  milancode model [MODEL_ID]                   Choose or persist a default model");
    println!("  milancode provider [PROVIDER_ID|default]     Choose a provider for the active model");
    println!("  milancode proxy [on|off|status]              Toggle XML tool-call proxy mode");
    println!(
        "  milancode mcp [status|tools|reload|add <name>|enable <name>|disable <name>] Inspect configured MCP servers and tools"
    );
    println!("  milancode plugins [list|help|install|enable|disable|uninstall|update] [TARGET]");
    println!("  milancode branch [list|create|switch] [ARG]  Inspect or change git branches");
    println!("  milancode worktree [list|add|remove|prune]   Inspect or manage git worktrees");
    println!("  milancode agents [list|help]                 List configured MilanCode agents");
    println!("  milancode skills [list|help|init <name>]     List or scaffold MilanCode skills");
    println!("  milancode init                               Create starter MilanCode project files");
    println!("  milancode doctor                             Run local environment diagnostics");
    println!("  milancode self-update                        Update from GitHub releases");
    println!("  milancode resume [SESSION_ID_OR_PATH]");
    println!("                                               Resume a saved session, or pick one and enter the REPL");
    println!("  milancode --resume [SESSION_ID_OR_PATH] [/status] [/compact] [...]");
    println!("                                               Resume a saved session and optionally run slash commands");
    println!(
        "  milancode prompt [--model MODEL] [--permission-mode MODE] [--mode MODE] [--reasoning LEVEL] [--fast] [--output-format text|json] TEXT"
    );
    println!(
        "                                               Send one prompt and stream the response"
    );
    println!(
        "  milancode [--model MODEL] [--permission-mode MODE] [--mode MODE] [--reasoning LEVEL] [--fast] [--output-format text|json] TEXT"
    );
    println!(
        "                                               Shorthand non-interactive prompt mode"
    );
    println!("  milancode dump-manifests");
    println!("  milancode bootstrap-plan");
    println!("  milancode system-prompt [--cwd PATH] [--date YYYY-MM-DD]");
    println!("  milancode --version");
    println!(
...
fn initialize_repo_for_model(
    ...
    if cwd.join("MILANCODE.md").exists() {
        return Ok((initialize_repo(cwd)?, None));
    }

    match generate_milancode_md(cwd, service, model) {
        Ok(content) => Ok((initialize_repo_with_milancode_md(cwd, &content)?, None)),
        Err(error) => Ok((
            initialize_repo(cwd)?,
...
fn generate_milancode_md(
    ...
    let request = MessageRequest {
        model: model.to_string(),
        max_tokens: INIT_MILANCODE_MD_MAX_TOKENS,
...
fn build_init_generation_prompt(cwd: &Path) -> Result<String, Box<dyn std::error::Error>> {
    ...
    writeln!(
        &mut prompt,
        "Create a project-specific `MILANCODE.md` for this repository."
    )?;
    writeln!(
        &mut prompt,
    ...
    writeln!(&mut prompt, "- `# MILANCODE.md`")?;
    ...
    writeln!(&mut prompt, "{}", render_init_milancode_md(cwd))?;
    writeln!(&mut prompt, "```")?;
...
fn collect_init_context_files(
    ...
    for file in &project_context.instruction_files {
        if let Some(name) = file.path.file_name().and_then(|name| name.to_str()) {
            if name.eq_ignore_ascii_case("MILANCODE.md") {
                continue;
            }
        }
...
fn extract_generated_milancode_md(
    response: MessageResponse,
) -> Result<String, Box<dyn std::error::Error>> {
    ...
    normalize_generated_milancode_md(&text)
        .ok_or_else(|| "model returned no markdown content for MILANCODE.md".into())
}

fn normalize_generated_milancode_md(raw: &str) -> Option<String> {
    ...
    let mut content = unfenced.find("# MILANCODE.md").map_or_else(
        || unfenced.to_string(),
        |index| unfenced[index..].to_string(),
    );
    if !content.starts_with("# MILANCODE.md") {
        content = format!("# MILANCODE.md\n\n{content}");
    }
...
fn format_init_generation_warning(
    ...
    format!(
        "warning: failed to generate a repo-specific MILANCODE.md with {}/{}; used the starter template instead: {error}",
        service.display_name(),
        model
    )
}
    ...
    let Some(release) = fetch_latest_release()? else {
        println!(
            ...
            render_update_report(
                ...
                Some(VERSION),
                None,
                Some("GitHub latest release endpoint returned no published release for nanogpt-community/milancode."),
                None,
            )
        );
        return Ok(());
    ...
    #[cfg(windows)]
    {
        println!(
            ...
            render_update_report(
                ...
                Some(&latest_version),
                Some(&format!(
                    "Download {} from the latest GitHub release and replace milancode.exe manually. In-place self-update is not supported on Windows yet.",
...
fn release_asset_candidates() -> Vec<String> {
    let mut candidates = target_name_candidates()
        .into_iter()
        .flat_map(|target| {
            let mut names = vec![format!("milancode-{target}")];
            if env::consts::OS == "windows" {
                names.push(format!("milancode-{target}.exe"));
            }
            names
        })
        .collect::<Vec<_>>();
    if env::consts::OS == "windows" {
        candidates.push("milancode.exe".to_string());
    }
    candidates.push("milancode".to_string());
    candidates.sort();
    candidates.dedup();
    candidates
}
...
mod tests {
    use super::{
        ...
        login_model_guidance, normalize_generated_milancode_md, parse_args, parse_auth_command,
        ...
        LogoutCommand, McpCatalog, McpCommand, MilanCodeRuntimeClient, RuntimeToolSpec, StatusContext,
    ...
    fn permission_env_overrides_persisted_runtime_default() {
        with_isolated_config_home(|| {
            ...
            .expect("runtime defaults should persist");
            let original = std::env::var("MILANCODE_PERMISSION_MODE").ok();
            std::env::set_var("MILANCODE_PERMISSION_MODE", "read-only");

            let parsed = parse_args(&[]).expect("args should parse");

            match original {
                Some(value) => std::env::set_var("MILANCODE_PERMISSION_MODE", value),
                None => std::env::remove_var("MILANCODE_PERMISSION_MODE"),
            }
            assert_eq!(
                parsed,
                CliAction::Repl {
    ...
    fn runtime_client_constructor_defers_api_key_lookup() {
        with_isolated_config_home(|| {
            ...
            let client = MilanCodeRuntimeClient::new(
                ApiService::NanoGpt,
                DEFAULT_MODEL.to_string(),
                DEFAULT_MAX_TOKENS,
                None,
    ...
    #[test]
    fn nanogpt_login_guidance_is_suppressed_for_default_model() {
        with_isolated_config_home(|| {
            assert_eq!(login_model_guidance(AuthService::NanoGpt), None);
        });
    }
    ...
    fn parses_logout_subcommand() {
        with_isolated_config_home(|| {
            let args = vec!["logout".to_string(), "exa".to_string()];
            assert_eq!(
                parse_args(&args).expect("args should parse"),
                CliAction::Logout {
    ...
    fn parses_checksum_manifest_for_named_asset() {
        let manifest = "abc123 *milancode-aarch64-apple-darwin\ndef456 other-file\n";
        assert_eq!(
            parse_checksum_for_asset(manifest, "milancode-aarch64-apple-darwin"),
    ...
    fn select_release_assets_requires_checksum_file() {
        ...
        let release = GitHubRelease {
            ...
            assets: vec![GitHubReleaseAsset {
                name: asset_name,
                browser_download_url: "https://example.invalid/milancode".to_string(),
            }],
        };
    ...
    fn update_report_includes_changelog_when_present() {
        ...
        assert!(report.contains("nanogpt-community/milancode"));
    }
    ...
    fn parses_logout_slash_command() {
        ...
        assert_eq!(
            parse_logout_command("/logout exa"),
            Some(LogoutCommand {
                service: Some(AuthService::Exa),
            })
        );
    ...
    fn removes_saved_credentials_for_selected_service() {
        with_isolated_config_home(|| {
            let config_home =
                std::env::var("MILANCODE_CONFIG_HOME").expect("isolated config home should be set");
            ...
            let outcome = remove_saved_credentials(AuthService::Exa)
            ...
            assert!(parsed.get("exa_api_key").is_none());
            assert_eq!(
                parsed
                    .get("nanogpt_api_key")
                    .and_then(serde_json::Value::as_str),
    ...
    #[test]
    fn resolves_known_milancode_model_aliases() {
    ...
    fn archives_report_lists_archived_tool_results() {
        let temp = std::env::temp_dir().join(format!(
            "milancode-archives-list-{}",
            SystemTime::now()
                .duration_since(UNIX_EPOCH)
                .expect("time after epoch")
                .as_nanos()
        ));
        let session_path = temp.join(".milancode/sessions/test.json");
        let archive_path = temp.join(".milancode/tool-results/tool-1-bash.txt");
        ...
        {
            *archived_output_path = Some(".milancode/tool-results/tool-1-bash.txt".to_string());
        }
        let session = Session {
            version: 1,
    ...
    #[test]
    fn convert_messages_preserves_reasoning_for_assistant_messages() {
        let messages = vec![
    ...
    fn loads_custom_commands_from_config_and_command_dir() {
        ...
        std::fs::create_dir_all(temp.join(".milancode/commands")).expect("commands dir");
        std::fs::write(
            temp.join(".milancode/settings.json"),
            r#"{"command":{"review":{"template":"Review $ARGUMENTS","description":"review work"}}}"#,
        )
        .expect("settings write");
        std::fs::write(temp.join(".milancode/commands/fix.md"), "Fix $1").expect("command write");
    ...
    fn temp_fixture_dir(label: &str) -> PathBuf {
        let unique = SystemTime::now()
            ...
            .as_nanos();
        let path = std::env::temp_dir().join(format!("milancode-{label}-{unique}"));
    ...
    fn strip_markdown_code_fence_removes_wrapping_block() {
        ...
        assert_eq!(stripped, "# MILANCODE.md\n\nRules");
    }

    #[test]
    fn normalize_generated_milancode_md_adds_heading_when_missing() {
        let normalized = normalize_generated_milancode_md("Repository guidance")
            .expect("markdown should normalize");

        assert_eq!(normalized, "# MILANCODE.md\n\nRepository guidance\n");
    }

    #[test]
    fn normalize_generated_milancode_md_prefers_embedded_milancode_heading() {
        let normalized = normalize_generated_milancode_md(
