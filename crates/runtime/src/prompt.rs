...
fn discover_instruction_files(cwd: &Path) -> std::io::Result<Vec<ContextFile>> {
    ...
    for dir in directories {
        for candidate in [
            dir.join("MILANCODE.md"),
            dir.join("MILANCODE.local.md"),
            dir.join(".milancode").join("MILANCODE.md"),
            dir.join("CLAUDE.md"),
...
fn discover_memory_files(cwd: &Path) -> std::io::Result<Vec<ContextFile>> {
    ...
    for dir in discover_context_directories(cwd) {
        let memory_dir = dir.join(".milancode").join("memory");
...
fn render_project_context(project_context: &ProjectContext) -> String {
    ...
    if !project_context.instruction_files.is_empty() {
        bullets.push(format!(
            "MilanCode instruction files discovered: {}.",
...
fn render_instruction_files(files: &[ContextFile]) -> String {
    render_context_file_section("# MilanCode instructions", files)
}
...
fn render_config_section(config: &RuntimeConfig) -> String {
    ...
    if config.loaded_entries().is_empty() {
        lines.extend(prepend_bullets(vec![
            "No MilanCode settings files loaded.".to_string()
        ]));
        return lines.join("\n");
    }
...
mod tests {
    ...
    fn discovers_instruction_files_from_ancestor_chain() {
        ...
        fs::create_dir_all(nested.join(".milancode")).expect("nested milancode dir");
        fs::write(root.join("MILANCODE.md"), "root instructions").expect("write root instructions");
        fs::write(root.join("MILANCODE.local.md"), "local instructions")
            .expect("write local instructions");
        fs::create_dir_all(root.join("apps")).expect("apps dir");
        fs::write(root.join("apps").join("MILANCODE.md"), "apps instructions")
            .expect("write apps instructions");
        fs::write(root.join("apps").join("CLAUDE.md"), "compat instructions")
            .expect("write compat instructions");
        fs::write(nested.join(".milancode").join("MILANCODE.md"), "nested rules")
    ...
    fn dedupes_identical_instruction_content_across_scopes() {
        ...
        fs::write(root.join("MILANCODE.md"), "same rules\n\n").expect("write root");
        fs::write(nested.join("MILANCODE.md"), "same rules\n").expect("write nested");
    ...
    fn discovers_project_memory_files_from_ancestor_chain() {
        ...
        let root = temp_dir();
        let nested = root.join("apps").join("api");
        fs::create_dir_all(root.join(".milancode").join("memory")).expect("root memory dir");
        fs::create_dir_all(nested.join(".milancode").join("memory")).expect("nested memory dir");
        fs::write(
            root.join(".milancode").join("memory").join("2026-03-30.md"),
            "root memory",
        )
        .expect("write root memory");
        fs::write(
            nested.join(".milancode").join("memory").join("2026-03-31.md"),
            "nested memory",
        )
    ...
    fn displays_context_paths_compactly() {
        assert_eq!(
            display_context_path(Path::new("/tmp/project/.milancode/MILANCODE.md")),
            "MILANCODE.md"
        );
    }
    ...
    fn discover_with_git_includes_status_snapshot() {
        ...
        fs::write(root.join("MILANCODE.md"), "rules").expect("write instructions");
        ...
        assert!(status.contains("?? MILANCODE.md"));
    ...
    #[test]
    fn load_system_prompt_reads_milancode_files_and_config() {
        let _guard = test_env_lock();
        let root = temp_dir();
        fs::create_dir_all(root.join(".milancode")).expect("milancode dir");
        let home = root.join("home");
        let milancode_home = home.join(".milancode");
        fs::create_dir_all(&milancode_home).expect("milancode home");
        fs::write(root.join("MILANCODE.md"), "Project rules").expect("write instructions");
        fs::write(root.join("CLAUDE.md"), "Compat instructions")
            .expect("write compat instructions");
        fs::write(
            root.join(".milancode").join("settings.json"),
            r#"{"permissionMode":"acceptEdits"}"#,
        )
        .expect("write settings");
        fs::write(
            milancode_home.join("settings.json"),
        ...
        let previous_milancode_home = std::env::var_os("MILANCODE_CONFIG_HOME");
...
",
        ...
        if let Some(value) = previous_milancode_home {
            std::env::set_var("MILANCODE_CONFIG_HOME", value);
        } else {
            std::env::remove_var("MILANCODE_CONFIG_HOME");
        }
    ...
    #[test]
    fn renders_milancode_style_sections_with_project_context() {
        let root = temp_dir();
        fs::create_dir_all(root.join(".milancode")).expect("milancode dir");
        fs::write(root.join("MILANCODE.md"), "Project rules").expect("write MILANCODE.md");
        fs::write(
            root.join(".milancode").join("settings.json"),
            r#"{"permissionMode":"acceptEdits"}"#,
        )
        ...
        assert!(prompt.contains("# MilanCode instructions"));
        assert!(prompt.contains("Project rules"));
        assert!(prompt.contains("permissionMode"));
    ...
    fn renders_instruction_file_metadata() {
        let rendered = render_instruction_files(&[ContextFile {
            path: PathBuf::from("/tmp/project/MILANCODE.md"),
            content: "Project rules".to_string(),
        }]);
        assert!(rendered.contains("# MilanCode instructions"));
