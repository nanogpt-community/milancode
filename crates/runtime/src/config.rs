...
use platform::milancode_config_home_or_default;

use crate::json::JsonValue;
use crate::sandbox::{FilesystemIsolationMode, SandboxConfig};

pub const MILANCODE_SETTINGS_SCHEMA_NAME: &str = "SettingsSchema";
    ...
    pub fn discover(&self) -> Vec<ConfigEntry> {
        vec![
            ...
            ConfigEntry {
                ...
                path: self.cwd.join(".milancode").join("settings.json"),
            },
            ConfigEntry {
                source: ConfigSource::Local,
                path: self.cwd.join(".milancode").join("settings.local.json"),
            },
        ]
    }
...
fn apply_compaction_env_overrides(config: &mut RuntimeCompactionConfig) {
    if env_truthy("MILANCODE_DISABLE_AUTOCOMPACT") || env_truthy("OPENCODE_DISABLE_AUTOCOMPACT") {
        config.auto = false;
    }
    if env_truthy("MILANCODE_DISABLE_PRUNE") || env_truthy("OPENCODE_DISABLE_PRUNE") {
        config.prune = false;
    }
}
...
mod tests {
    use super::{
        ...
        MILANCODE_SETTINGS_SCHEMA_NAME,
    };
    use crate::json::JsonValue;
    ...
    fn rejects_non_object_settings_files() {
        ...
        let home = root.join("home").join(".milancode");
    ...
    #[test]
    fn loads_and_merges_milancode_config_files_by_precedence() {
        let root = temp_dir();
        let cwd = root.join("project");
        let home = root.join("home").join(".milancode");
        fs::create_dir_all(cwd.join(".milancode")).expect("project config dir");
        fs::create_dir_all(&home).expect("home config dir");
        ...
        fs::write(
            cwd.join(".milancode").join("settings.json"),
            r#"{"env":{"B":"2"},"hooks":{"PostToolUse":["project"]}}"#,
        )
        .expect("write project settings");
        fs::write(
            cwd.join(".milancode").join("settings.local.json"),
        ...
        assert_eq!(MILANCODE_SETTINGS_SCHEMA_NAME, "SettingsSchema");
